...
const init = async (ignitionOptions: NostrIgnitionOptions) => {
    ...
    if (!(window as any).nostr) {
        ...
        const RESPONSE_TIMEOUT = ignitionOptions.timeout || 7000; // 7 seconds
...
const logout = (): void => {
    localStorage.removeItem("localNostrPubkey");
    localStorage.removeItem("localNostrPrivkey");
    hasConnected = false;
    nip46.remotePubkey = null;
